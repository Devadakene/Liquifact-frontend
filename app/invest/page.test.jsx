...
// ── State Mutual Exclusivity Tests ──────────────────────────────────────────
describe("InvestMarketplace — State Mutual Exclusivity", () => {
  beforeEach(() => {
    jest.useFakeTimers();
  });
  ...
  it("loading state: renders skeleton, never renders error, empty, or list", async () => {
    const loadInvoices = createPendingLoader();
    render(<InvestMarketplace loadInvoices={loadInvoices} />);

    // Skeleton should be present
    expect(screen.getByRole("list", { name: /loading/i })).toBeInTheDocument();

    // Error banner, empty state, and invoice list should not be present
    expect(screen.queryByRole("alert")).not.toBeInTheDocument();
    expect(screen.queryByText(/No invoices available/i)).not.toBeInTheDocument();
    expect(screen.queryByRole("list", { name: /investable invoices/i })).not.toBeInTheDocument();
  });

  it("error state: renders alert, never renders skeleton, empty, or list", async () => {
    const loadInvoices = jest.fn(
      () =>
        new Promise((_, reject) => {
    ...
    // Error banner should be present with role="alert"
    expect(screen.getByRole("alert")).toBeInTheDocument();

    // Skeleton, empty state, and invoice list should not be present
    expect(screen.queryByRole("list", { name: /loading/i })).not.toBeInTheDocument();
    expect(screen.queryByText(/No invoices available/i)).not.toBeInTheDocument();
    expect(screen.queryByRole("list", { name: /investable invoices/i })).not.toBeInTheDocument();
  });

  it("empty state: renders EmptyState component, never renders skeleton, error, or list", async () => {
    render(<InvestMarketplace loadInvoices={createDeferredLoader([], 50)} />);
    await flushTimers(50);

    // Empty state should be present (EmptyState renders a section with specific text)
    expect(screen.getByText(/No invoices available/i)).toBeInTheDocument();
    expect(screen.getByText(/No investable invoices are currently available/i)).toBeInTheDocument();

    // Skeleton, error banner, and invoice list should not be present
    expect(screen.queryByRole("list", { name: /loading/i })).not.toBeInTheDocument();
    expect(screen.queryByRole("alert")).not.toBeInTheDocument();
    expect(screen.queryByRole("list", { name: /investable invoices/i })).not.toBeInTheDocument();
  });

  it("success state: renders invoice list, never renders skeleton, error, or empty", async () => {
    const invoices = makeInvoices(3);
    render(<InvestMarketplace loadInvoices={createDeferredLoader(invoices, 50)} />);
    await flushTimers(50);

    // Invoice list should be present
    expect(screen.getByRole("list", { name: /investable invoices/i })).toBeInTheDocument();
    expect(getInvoiceListItems()).toHaveLength(3);

    // Skeleton, error banner, and empty state should not be present
    expect(screen.queryByRole("list", { name: /loading/i })).not.toBeInTheDocument();
    expect(screen.queryByRole("alert")).not.toBeInTheDocument();
    expect(screen.queryByText(/No invoices available/i)).not.toBeInTheDocument();
  });

  it("no-match state (after filter): renders EmptyState, never renders list with invoices", async () => {
    const invoices = [
      {
        id: "inv-001",
        issuer: "A Corp",
    ...
    // Apply filter that eliminates all invoices
    fireEvent.click(screen.getByLabelText("Filter by EUR"));

    // Empty state for no-match should be present
    expect(screen.getByText("No invoices match your filters.")).toBeInTheDocument();

    // Invoice list should not be present
    expect(screen.queryByRole("list", { name: /investable invoices/i })).not.toBeInTheDocument();
  });
});

// ── Accessibility Tests for Empty/Error States ──────────────────────────────
describe("InvestMarketplace — Accessibility (Empty/Error States)", () => {
  beforeEach(() => {
    jest.useFakeTimers();
  });
  ...
  it("empty state: section has aria-label, aria-live, and role=status", async () => {
    render(<InvestMarketplace loadInvoices={createDeferredLoader([], 50)} />);
    await flushTimers(50);

    const emptySection = screen.getByText(/No invoices available/).closest("section");
    expect(emptySection).toHaveAttribute("aria-label", "Marketplace empty state");
    expect(emptySection).toHaveAttribute("aria-live", "polite");
    expect(emptySection).toHaveAttribute("role", "status");
    expect(emptySection).toHaveAttribute("aria-atomic", "true");
  });

  it("error state: section has aria-label and aria-live=assertive", async () => {
    const loadInvoices = jest.fn(
      () =>
        new Promise((_, reject) => {
          setTimeout(() => reject(new Error("load failed")), 50);
    ...
    const errorSection = screen.getByRole("alert").closest("section");
    expect(errorSection).toHaveAttribute("aria-label", "Marketplace error state");
    expect(errorSection).toHaveAttribute("aria-live", "assertive");
    expect(errorSection).toHaveAttribute("aria-atomic", "true");
  });

  it("error banner: has role=alert (already set in ErrorBanner component)", async () => {
    const loadInvoices = jest.fn(
      () =>
        new Promise((_, reject) => {
          setTimeout(() => reject(new Error("load failed")), 50);
  ...
  it("retry button: is a real button with proper keyboard support", async () => {
    const loadInvoices = jest.fn(
      () =>
        new Promise((_, reject) => {
          setTimeout(() => reject(new Error("load failed")), 50);
    ...
    // Button should be a real button element
    expect(retryButton.tagName).toBe("BUTTON");

    // Button should be in tab order (focusable)
    expect(retryButton).not.toHaveAttribute("tabindex", "-1");

    // Button should have a clear accessible name
    expect(retryButton).toHaveAccessibleName(/try again/i);
  });

  it("retry button: Enter key triggers refetch", async () => {
    let callCount = 0;
    const invoices = makeInvoices(1);
    const loadInvoices = jest.fn(() => {
      callCount += 1;
      ...
      return new Promise((resolve) => {
        setTimeout(() => resolve(invoices), 50);
      });
    });
    ...
    // Simulate keyboard: focus and press Enter
    retryButton.focus();
    ...
    // Button should trigger the retry action (loadInvoices called twice: initial + retry)
    ...
    // Should show list after successful load
    expect(getInvoiceListItems()).toHaveLength(1);
  });

  it("retry button: Space key triggers refetch", async () => {
    let callCount = 0;
    const invoices = makeInvoices(1);
    const loadInvoices = jest.fn(() => {
      callCount += 1;
    ...
    // Simulate keyboard: focus and press Space
    retryButton.focus();
    expect(document.activeElement).toBe(retryButton);
    ...
    // Button should trigger the retry action
    expect(loadInvoices).toHaveBeenCalledTimes(2);
    expect(screen.getByRole("list", { name: /loading/i })).toBeInTheDocument();
  ...
  it("no-match state: section has aria-label and aria-live=polite", async () => {
    const invoices = [
      {
        id: "inv-001",
        issuer: "A Corp",
    ...
    const noMatchSection = screen.getByText(/No invoices match/).closest("section");
    expect(noMatchSection).toHaveAttribute("aria-label", "Marketplace no-match state");
    expect(noMatchSection).toHaveAttribute("aria-live", "polite");
    expect(noMatchSection).toHaveAttribute("aria-atomic", "true");
  });
});

// ── Transition Sequence Tests ──────────────────────────────────────────────────
describe("InvestMarketplace — State Transition Sequences", () => {
  beforeEach(() => {
    jest.useFakeTimers();
  });
  ...
  it("loading → error → retry → loading → success sequence", async () => {
    let callCount = 0;
    const invoices = makeInvoices(2);
    const loadInvoices = jest.fn(() => {
      callCount += 1;
    ...
    // 4. loading state (skeleton reappears)
    expect(screen.getByRole("list", { name: /loading/i })).toBeInTheDocument();
    expect(screen.queryByRole("alert")).not.toBeInTheDocument();

    await flushTimers(50);

    // 5. success state (list renders)
    expect(screen.queryByRole("alert")).not.toBeInTheDocument();
    expect(getInvoiceListItems()).toHaveLength(2);
  });

  it("loading → empty sequence", async () => {
    render(<InvestMarketplace loadInvoices={createDeferredLoader([], 50)} />);
  ...
  it("success → no-match (via filter) sequence", async () => {
    const invoices = [
      {
        id: "inv-001",
        issuer: "USD Corp",
    ...
    // 1. success state (list visible)
    expect(getInvoiceListItems()).toHaveLength(1);

    // 2. apply filter that matches no invoices
    fireEvent.click(screen.getByLabelText("Filter by EUR"));

    // 3. no-match state (EmptyState visible)
